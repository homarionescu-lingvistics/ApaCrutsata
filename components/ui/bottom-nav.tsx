...
export function BottomNav() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-slate-800 bg-slate-950/95 backdrop-blur pb-safe">
      <ul className="mx-auto flex max-w-lg items-stretch justify-around px-1 pt-1">
        {items.map(({ href, label, sub, icon: Icon }) => (
          <li key={href} className="flex-1">
            <Link
              href={href}
              className="flex flex-col items-center gap-0.5 rounded-lg px-1 py-2 text-[10px] text-slate-400 transition active:scale-95 hover:bg-slate-900 hover:text-slate-100"
            >
              <Icon className="h-5 w-5" aria-hidden />
              <span className="font-medium leading-none">{label}</span>
              <span className="text-[9px] leading-none text-slate-500">{sub}</span>
            </Link>
          </li>
        ))}
      </ul>
